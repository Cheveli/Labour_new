...
export default function ReportsPage() {
  ...
  const exportProjectPDF = () => {
    ...
    doc.save(`Project_Attendance_${projName}_${format(new Date(startDate), 'dd-MMM')}_to_${format(new Date(endDate), 'dd-MMM-yyyy')}.pdf`)
    toast.success('PDF exported')
  }
            ...
            <div className="bg-[#111520] border border-zinc-800 rounded-2xl overflow-hidden shadow-2xl">
              {activeTab === 'INDIVIDUAL' && (
                <div className="p-8 space-y-8">
                  ...
                  {/* Mobile Cards for INDIVIDUAL */}
                  <div className="flex flex-col gap-3 md:hidden">
                    {reportData.breakdown.map((row: any, i: number) => (
                      <div key={i} className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4 flex flex-col gap-3">
                        <div className="flex justify-between items-center">
                          <p className="font-bold text-white text-sm">{format(new Date(row.date), 'EEEE (dd MMM)')}</p>
                          <Badge className={cn("px-2 py-0.5 rounded text-[8px] font-black uppercase border-none", row.status === 'ABSENT' ? "bg-red-500/20 text-red-500" : "bg-emerald-500/20 text-emerald-400")}>
                            {row.status}
                          </Badge>
                        </div>
                        <div className="flex justify-between items-end">
                          <div>
                            <p className="text-[10px] font-bold text-zinc-500 uppercase">Project</p>
              ...
              {activeTab === 'PROJECT_WEEKLY' && (
                <div className="p-0">
                  ...
                  {/* Mobile Cards for PROJECT_WEEKLY */}
                  <div className="flex flex-col gap-4 p-4 md:hidden">
                    {reportData.workers.map((w: any, idx: number) => (
                      <div key={idx} className="bg-[#1a2030] border border-zinc-800 rounded-xl p-4 flex flex-col gap-3 shadow-lg">
                    ...
                    {reportData.items.map((r: any, i: number) => (
                      <div key={i} className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4 flex flex-col gap-3">
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="font-black text-white text-xs uppercase tracking-tight">{r.name}</p>
                        ...
                        {r.notes && <p className="text-xs text-zinc-500 italic mt-1 bg-zinc-950 p-2 rounded">{r.notes}</p>}
                      </div>
                    ))}
                   </div>
                </div>
